...
<html lang="no">
  ...
  <body>
    ...
    <div id="root">
      <form method="post">
        <fieldset>
          <label for="fornavn">Fornavn:</label> <input type="text" name="fornavn" id="fornavn" value="${deltagerForm.fornavn}" onkeypress="fornavnSjekk()" />
          <span class="melding">${deltagerForm.fornavnMelding}</span>

          <label for="etternavn">Etternavn:</label> <input type="text" name="etternavn" id="etternavn" value="${deltagerForm.etternavn}" onkeypress="etternavnSjekk()"/>
          <span class="melding">${deltagerForm.etternavnMelding}</span>

          <label for="mobil">Mobil (8 siffer):</label> <input type="text" name="mobil" id="mobil" value="${deltagerForm.mobil}" onkeypress="mobilSjekk()"/>
          <span class="melding">${deltagerForm.mobilMelding}</span>

          <label for="passord">Passord:</label> <input type="password" name="passord" id="passord" value="${deltagerForm.passord}" onkeypress="passordSjekk()" onmouseover="musOver()"/>
          <span class="melding">${deltagerForm.passordMelding}</span>

          <label for="passordRepetert">Passord repetert:</label> <input type="password" name="passordRepetert" id="passordRepetert" value="${deltagerForm.passordRepetert}" onkeypress="passordLikt()"/>
